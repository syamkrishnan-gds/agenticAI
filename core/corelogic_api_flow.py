...
from config.config import CBTA_API_KEY,Model_Name,Api_Version,Azure_Endpoint,chatbot_endpoint, chatbot_api_key, ado_org, ado_project, ado_pat_token,AZURE_SEARCH_ENDPOINT, AZURE_SEARCH_INDEX_NAME, AZURE_SEARCH_API_KEY
from utils.tasks import TEST_MANAGER_TASK, UTTERANCES_GENERATOR_TASK, UTTERANCES_REVIEWER_TASK, CHATBOT_TESTER_TASK, ADO_HELPER_TASK,AZURE_SEARCH_VALIDATOR_TASK
...
# Calling Azure AI Search to validate the utterance
def query_azure_ai_search(utterance: str) -> str:
    ...
    try:
        ...
        if count == 0:
            return "No relevant grounded information found in Azure AI Search."

        # Retrieve and log the first result for debugging
        first_result = next(results, None)
        if first_result:

            # Adjust 'content' to match your actual field name in Azure AI Search index
            content_field_name = 'content'
            grounded_info = first_result.get(content_field_name, None)
            if grounded_info:
                return grounded_info
            else:
                return f"Relevant document found, but '{content_field_name}' field is missing or empty."
        else:
            return "No relevant grounded information found despite search count > 0."
...
async def start_workflow(user_query):
    ...
    Azure_Search_Query_Tool = FunctionTool(
        query_azure_ai_search,
        description="Queries Azure AI Search with an utterance to retrieve relevant grounded information."
    )

    # | Agent                    | Temperature | Top_k | Top_p | Rationale                       |
    # |--------------------------|-------------|-------|-------|---------------------------------|
    # | Azure Search Validator   | 0.3         | 50    | 0.8   | Factual comparison focus        |
    asv_model_client = AzureOpenAIChatCompletionClient(
        azure_deployment=Model_Name,
        model=Model_Name,
        api_version=Api_Version,
        azure_endpoint=Azure_Endpoint,
    ...
    # Azure Search Validator Agent - New Agent
    Azure_Search_Validator_Agent = AssistantAgent(
        "Azure_Search_Validator_Agent",
        model_client=asv_model_client,
        tools=[Azure_Search_Query_Tool], # Provide the search tool
        system_message=AZURE_SEARCH_VALIDATOR_TASK,
    )
    ...
    team = SelectorGroupChat(
        [Test_Manager_Agent, Utterances_Generator_Agent, Utterances_Reviewer_Agent, Chatbot_Tester_Agent,Azure_Search_Validator_Agent,ADO_Helper_Agent],
        model_client=tm_model_client,
